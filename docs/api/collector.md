...
### 超时配置说明
系统支持多层级的超时配置，优先级如下：
1. **API参数级**: `device_timeout` > `task_timeout`
2. **配置文件级**: 平台特定的 `timeout_all` > 全局 `ssh.timeout` > 默认值(60秒)

`timeout_all` 是配置文件中的新参数，用于控制设备交互的总超时时间。详细配置方法请参考 [配置文档](../configuration.md)。
