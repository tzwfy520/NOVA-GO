...
const (
	enabledParamTrue  = "true"
	enabledParamFalse = "false"
)
...
func getDeviceByAnyID(db *gorm.DB, idStr string, device *model.DeviceInfo) bool {
	if device == nil {
		return false
	}
...
func (h *DeviceHandler) GetDevice(c *gin.Context) {
	...
	db := database.GetDB()
	var device model.DeviceInfo
	if !getDeviceByAnyID(db, idStr, &device) {
...
func (h *DeviceHandler) ListDevices(c *gin.Context) {
	...
	if enabledParam != "" {
		switch enabledParam {
		case enabledParamTrue, "1":
			query = query.Where("enabled = ?", true)
		case enabledParamFalse, "0":
